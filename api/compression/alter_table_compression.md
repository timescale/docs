...
| `timescaledb.enable_sparse_index_bloom`    |BOOLEAN| A [bloom filter][bloom-filters] is an additional sparse index, like min-max, that optimizes queries for faster retrieval. From TimescaleDB 2.20 and higher, bloom filters are enabled by default. For data compressed using TimescaleDB 2.19.3 and lower, you need to recompress data to enable bloom filters.  |
