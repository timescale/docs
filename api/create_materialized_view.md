...
|`timescaledb.finalized`|BOOLEAN|In TimescaleDB 2.7 and above, use the new version of continuous aggregates, which stores finalized results for aggregate functions. Supports non-parallelizable aggregate functions, `FILTER` and `JOIN` clauses, and the `DISTINCT` keyword.|`TRUE`|
