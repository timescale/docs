...
- Improve continuous aggregate performance by collecting invalidations from the WAL <Since2220 />:

   ```sql
   ALTER MATERIALIZED VIEW contagg_view SET (timescaledb.invalidate_using = 'wal');
   ```
...
| `timescaledb.invalidate_using`                                            | TEXT      | `trigger`                                            | ✖        | <Since2220 /> Set to `wal` to read changes from the WAL using logical decoding, then update the materialization invalidations for continuous aggregates using this information.  This reduces the I/O and CPU needed to manage the hypertable invalidation log.|
...
[decompress]: /api/:currentVersion:/compression/decompress_chunk/
