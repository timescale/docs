...
per project and per region setting. This means that all $MST_SERVICE_SHORTs created and
...
means that all $MST_SERVICE_SHORTs created and running use the same VPC peering connection.
...
1.  In [$MST_CONSOLE_SHORT][mst-login], click `VPC`.
...
1.  In the `IP range` field, type the IP range that you want to use for the VPC connection.
    ...
    you could set the range for your $MST_LONG project VPC to 192.168.0.0/24.
