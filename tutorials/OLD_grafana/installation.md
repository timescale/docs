...
You need to sign in to your $MST_LONG to create a
new $SERVICE_SHORT to run Grafana.

<Procedure>

### Creating a new service for Grafana

1.  Log in to your $MST_LONG account, and click `Create a new service`.
...
1.  When you are happy with your selections, click `Create Service` to go back
    to the `Services` view while your $SERVICE_SHORT is created. The status indicator
    says `Rebuilding` while the $SERVICE_SHORT is created. It is ready for you to use
    when the indicator is green and says `Running`. This usually takes a couple
    of minutes, but different clouds can vary. You can click the $SERVICE_SHORT name in
...
When your $SERVICE_SHORT is built, you can log and set up your data services.

### Logging in to your MST Grafana service

1.  In the $MST_LONG `Services` view, click the name of
    your new Grafana service.
1.  On the $SERVICE_SHORT details page, take a note of the user name and password for
