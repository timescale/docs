...
![Project users in $CONSOLE_LONG](https://assets.timescale.com/docs/images/console-users-overview.png)
...
New users do not need to have a $CONSOLE_SHORT account before you add them, they are
prompted to create one when they respond to the confirmation email. Existing users
join a project in addition to the other $PROJECT_SHORTs they are already members of.

To add a user to a $PROJECT_SHORT:

<Procedure>

1.  In [$CONSOLE_SHORT][cloud-login], click `Invite users`, then click `Add new user`.
...
1.  Type the email address of the person that you want to add, and click `Add
    ...
    [Enterprise plan][pricing-plans] and SAML users receive a notification in $CONSOLE_SHORT. Users in the 
...
When you are asked to join a $PROJECT_SHORT, the $CONSOLE_SHORT sends you an invitation email. Follow the
instructions in the invitation email to join the $PROJECT_SHORT:

- **New $CLOUD_LONG user**:
  1. In the invitation email, click **Accept Invite**.
     $CLOUD_LONG opens.
  2. Follow the setup wizard and create a new $ACCOUNT_LONG.
...
- **Existing $CLOUD_LONG user**:
  1. In the invitation email, click **Accept Invite**.
     ...
     $CLOUD_LONG opens and you are added to the $PROJECT_SHORT.
...
Project invitations are valid for 7 days. To resend a $PROJECT_SHORT invitation:

<Procedure>

1.  In the [$CONSOLE_SHORT][cloud-login], click `Invite users`.
...
1. In the [$CONSOLE_SHORT][cloud-login], click `Tiger Cloud Project`, then `Current project`.
...
Each $PROJECT_SHORT in $CONSOLE_SHORT has one `owner`. As the $PROJECT_SHORT owner, you have rights to
...
1.  In [$CONSOLE_SHORT][cloud-login], click `Invite users`.
...
1.  In [$CONSOLE_SHORT][cloud-login], click `Invite users`.
1.  Next to the person you want to remove, click `⋮`, then click the trash icon.
