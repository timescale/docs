...
* The $S3_CONNECTOR offers an option to enable a [$HYPERTABLE][hypertables-section] during the file-to-table schema mapping setup. You can enable [columnstore][compression] and [continuous aggregates][caggs] through the $SQL_EDITOR once the connector has started running.
