...
const checkTerminalIndent = (params, onError) => {
  ...
  const { lines } = params;
  const terminalOpeningTags =
    findPatternInLines(lines, '<terminal>');
  const terminalClosingTags =
    findPatternInLines(lines, '<\/terminal>');
  const tabOpeningTags =
    findPatternInLines(lines, '<tab.+>');
  const tabClosingTags =
    findPatternInLines(lines, '<\/tab>');
  if (terminalOpeningTags.length === 0) return;

  // Check that the number of opening and closing terminal tags match
  checkTagsClosed(
    terminalOpeningTags,
    terminalClosingTags,
    onError,
  ...
  for (let i = 0; i < terminalOpeningTags.length; i++) {
    ...
    const enclosedTabs = tabOpeningTags.filter((tag) => 
      isBetween(tag, terminalOpeningLine, terminalClosingLine)
    ).concat(tabClosingTags.filter((tag) =>
      isBetween(tag, terminalOpeningLine, terminalClosingLine)
    ));
