...
| `debug_bgw_scheduler_exit_status` | `INTEGER` | `0` | min: `0`, max: `255` |  this is for debugging purposes |
...
| `debug_toast_tuple_target` | `INTEGER` | `/* bootValue = */ 128` | min: `/* minValue = */ 1`, max: `/* maxValue = */ 65535` |  this is for debugging purposes |
...
| `hypercore_arrow_cache_max_entries` | `INTEGER` | `25000` | min: `1`, max: `INT_MAX` | The max number of decompressed arrow segments that can be cached before entries are evicted. This mainly affects the performance of index scans on the Hypercore TAM when segments are accessed in non-sequential order. |
