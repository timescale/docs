For $TIMESCALE_DB [v2.23.0](tsdb-release-2-23-0) and higher, the table is automatically partitioned on the first column in the table with a 
timestamp data type. 
If multiple columns are suitable candidates as a partitioning column, $TIMESCALE_DB will throw an error and ask for an explicit definition.
For earlier versions, set `partition_column` to a time column. 

If you are self-hosting $TIMESCALE_DB [v2.20.0](tsdb-release-2-23-0) to [v2.22.1](tsdb-release-2-23-0), to convert your data to the $COLUMNSTORE after a specific time 
