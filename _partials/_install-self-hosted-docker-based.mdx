...
1.  **Run the $TIMESCALE_DB Docker image**
    ...
    The [TimescaleDB HA](https://hub.docker.com/r/timescale/timescaledb-ha) Docker image offers the most complete $TIMESCALE_DB experience. It uses [Ubuntu][ubuntu], includes [$TOOLKIT_LONG](https://github.com/timescale/timescaledb-toolkit), and support for PostGIS and Patroni. The latest release based on PostgreSQL 17 is `timescale/timescaledb-ha:pg17`:

    ```bash
    docker pull timescale/timescaledb-ha:pg17
    ```

    The lighter-weight [TimescaleDB](https://hub.docker.com/r/timescale/timescaledb) non-ha Docker image uses [Alpine][alpine]. The latest release based on PostgreSQL 17 is `timescale/timescaledb:latest-pg17`.
