...
$HYPERTABLE_CAPs are $PG tables in $TIMESCALE_DB that automatically partition your time-series data by time. Time-series data represents the way a system, process, or behavior changes over time. $HYPERTABLE_CAPs enable $TIMESCALE_DB to work efficiently with time-series data.  Each $HYPERTABLE is made up of child tables called chunks. Each chunk is assigned a range 
...
Because $TIMESCALE_DB is 100% $PG, you can use all the standard $PG tables, indexes, stored
