...
1. **Configure $PG**
    ...
    If you have more than one version of $PG installed, $TIMESCALE_DB can only
    be associated with one of them. The $TIMESCALE_DB build scripts use `pg_config` to
    find out where $PG stores its extension files, so you can use `pg_config`
    to find out which $PG installation $TIMESCALE_DB is using.
