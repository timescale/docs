...
docker run --rm -it --name live-migration \
    ...
    timescale/live-migration:v0.0.22 --help
...
docker run --rm -it --name live-migration-snapshot \
    ...
    --pid=host \
    -v ~/live-migration:/opt/timescale/ts_cdc \
    timescale/live-migration:v0.0.22 snapshot
```
...
docker run --rm -it --name live-migration-migrate \
    ...
    --pid=host \
    -v ~/live-migration:/opt/timescale/ts_cdc \
    timescale/live-migration:v0.0.22 migrate --help
...
docker run --rm -it --name live-migration-migrate \
    ...
    --pid=host \
    -v ~/live-migration:/opt/timescale/ts_cdc \
    timescale/live-migration:v0.0.22 migrate
```
<Highlight type="note">
