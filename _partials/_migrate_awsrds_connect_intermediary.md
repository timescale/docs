...
1. In [https://console.aws.amazon.com/rds/home#databases:][databases],
   select the RDS/Aurora $PG instance to migrate.
1. Click `Actions` > `Set up EC2 connection`.
   Press `Create EC2 instance` and use the following settings:
    ...
    - **Configure Storage**: adjust the volume to at least the size of RDS/Aurora $PG instance you are migrating from.
...
1. On your intermediary EC2 instance, install the $PG client.
  ...
  Keep this terminal open, you need it to connect to the RDS/Aurora $PG instance for migration. 

</Procedure>

## Set up secure connectivity between your RDS/Aurora $PG and EC2 instances
...
1. On your intermediary EC2 instance, get your local IP address:
   ...
   ```
   Bear with me on this one, you need this IP address to enable access to your RDS/Aurora $PG instance.
...
## Test the connection between your RDS/Aurora $PG and EC2 instances
...
1. On your intermediary EC2 instance, use the values of `Endpoint`, `Port`, `Master username`, and `DB name`  
   ...
   ```
   The value of `Master password` was supplied when this RDS/Aurora $PG instance was created.

1. Test your connection:
   ```sh
   psql -d $SOURCE 
   ```
   You are connected to your RDS/Aurora $PG instance from your intermediary EC2 instance.
