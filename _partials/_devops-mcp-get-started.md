...
|         | start http       | Start the $MCP_SHORT with HTTP transport. This option is for users who wish to access $MCP_LONG without using stdio. For example, your AI assistant does not support stdio, or you do not want to run $CLI_SHORT on your device.<p/>    Flags are: <ul><li>`--port <port number>` The default is 8000.</li><li>`--host <hostname>` The default is localhost.</li></ul>            |
