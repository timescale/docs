...
For PostgreSQL users who may be familiar with views and materialized views, here's
how continuous aggregates differ:

* Unlike a PostgreSQL view, a continuous aggregate does not perform the
...
Now that you're familiar with what continuous aggregates are, let's create your
first continuous aggregate. Creating a continuous aggregate is a two-step
process. First define your view. Then, create a policy to schedule refreshes of
the continuous aggregate.

The following example creats a daily aggregation of all weather metrics.

### Step 1: Define view

Here's the SQL statement defining the query to be maintained in the continuous
aggregate. It calculates the daily average for all weather metrics, as well as
...
Notice how `time_bucket` is used to bucket the data into 1-day intervals.

That's your first continuous aggregate. Here are some ideas for other aggregates
to create:
...
You've created the continuous aggregate but it has not materialized any data.
There are two ways to populate a continuous aggregate: via manual refresh or an
automation policy. These methods enable you to refresh materialized data in your
...
You can manually refresh a continuous aggregate using a "one-shot refresh."
...
Let's examine an example of a query which would perform better on the continuous
aggregate (`weather_metrics_daily`) than on the hypertable (`weather_metrics`).
...
end date for which you materialized data in the continuous aggregate (done via
manual refresh earlier in this tutorial). Despite not materializing the latest
data via manual refresh or automation policy (since the policy created above
hasn't yet run), you still get the latest data thanks to real-time aggregation.
